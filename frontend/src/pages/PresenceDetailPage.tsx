...
import { ArrowLeft, MapPin, QrCode, Calendar } from "lucide-react";

const MOCK_USERS: Record<string, { name: string; sport: string; status: string; distance: string }> = {
  "1": { name: "Alex M.", sport: "Soccer", status: "Out Now", distance: "0.3 mi" },
  "2": { name: "Jordan K.", sport: "Basketball", status: "On My Way", distance: "0.7 mi" },
  "3": { name: "Sam R.", sport: "Tennis", status: "Planned", distance: "1.2 mi" },
  "4": { name: "Casey L.", sport: "Running", status: "Out Now", distance: "0.5 mi" },
  "5": { name: "Morgan T.", sport: "Cycling", status: "Out Now", distance: "0.9 mi" },
};
...
export default function PresenceDetailPage() {
  ...
  const user = (id && MOCK_USERS[id]) ? MOCK_USERS[id] : { name: "Unknown Buddy", sport: "Unknown", status: "Unknown", distance: "?" };
    ...
    <div className="p-4 max-w-lg mx-auto">
      ...
      <div className="bg-charcoal rounded-xl p-6 mb-4">
        <div className="flex items-start justify-between mb-4">
          ...
          <span className={`text-sm font-semibold ${statusColor}`}>{user.status}</span>
        </div>
