...
function loadState(): EventJoinState {
  try {
    ...
    const overrides = JSON.parse(
      safeGetItem(STORAGE_KEY_PARTICIPANT_OVERRIDES, "{}"),
    );
...
function saveState(state: EventJoinState): void {
  ...
  safeSetItem(
    STORAGE_KEY_PARTICIPANT_OVERRIDES,
    JSON.stringify(state.participantOverrides),
  );

  // Dispatch custom event for cross-tab/component synchronization
  if (typeof window !== "undefined") {
    window.dispatchEvent(
      new CustomEvent("eventJoinStateChanged", { detail: state }),
    );
  }
}
