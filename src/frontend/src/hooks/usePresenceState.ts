...
export function usePresenceState() {
  ...
  const loadPresenceState = useCallback(() => {
    const status = localStorage.getItem(PRESENCE_KEYS.status) as
      | "OUT_NOW"
      | null;
    ...
    return {
      ...
      presenceSport: sport,
      presenceActivatedAt: activatedAt
        ? Number.parseInt(activatedAt, 10)
        : null,
      presenceExpiresAt: expiresAt ? Number.parseInt(expiresAt, 10) : null,
      pocketFlashUntil: flashUntil ? Number.parseInt(flashUntil, 10) : null,
    };
  }, []);
